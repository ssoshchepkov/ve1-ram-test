...
; Инициализация каналов 0, 1 и 2
                        ...
                        out (CtcChannel2),A             ; Выполняем команду для канала 1. Канал не используется.

; Инициализация канала 3
; Канал делит CPU CLK на (256*255)
; Канал 3 вызывает прерывание CPU.
; Обработчик прерывания находится по адресу Channel3Vector
                        ld A, 10100111b                 ; параметры каналов (7-0 биты): прерывания вкл., режим
                                                        ; таймера, делитель 256, CLK/TRG Edge = Falling Edge, 
                                                        ; таймер запускается после загрузки константы, константа 
                                                        ; передается после выполнения этой команды, программный 
                                                        ; reset включен, это команда управления
                        out (CtcChannel3), A
                        ld A, 255                       ; задаем временную константу, равную FFh
